<?php 
	...
	// printArr(ProcessSamsungInvoice('sample_invoice/Samsung/INVOICE_1.pdf'));
		...
		foreach ($emails as $email_number) {
	        ...
	        if ($header->from[0]->host != 'sbe-ltd.ca') {
	        	...
	        	if ($sender != 'productionreport@sbe-ltd.co.uk') {
	        		...
	        		if ($sender === 'credit.seca@samsung.com') {
	        			# code...
						...
						foreach ($attachments as $attachment) {
							...
							if ($attachment['is_attachment'] == 1) {
								...
								if (in_array($extension, array('.pdf','.xls','.xlsx','.PDF', '.XLS','.XLSX'))) {
									...
									$check_qry = "SELECT * FROM CARLO_TEST WHERE email_subject = '{$data['email_subject']}' and attached_document = '{$filename}' and sender_email = '{$data['sender_email']}' and sent_date = '{$data['sent_date']}' and sent_time = '{$data['sent_time']}'";
			                		...
			                		if (empty($check_res)) {
						                ...
						                $document_folder = "sample_invoice";

...
function login($username, $password,$portal_name, $url) {
	...
	if (!$validateUser) {
		...
		$res = getUserData($username, $password);
		$user = new User($res);
...
function generateMultipleExcel2($data,$reportname){
	...
	*/
	...
	function getSupplier($sender, $domain) {
		include 'dbCon.php';
	...
	function errorNotification($email, $location, $query) {
		# send email notification whenever a sql error has occured
		require_once  'MailPack/PHPMailer/PHPMailerAutoload.php';
		require 'MailPack/phpCredential.php';
		$datein = date("Y-m-d"); 
		$timein	= date("G:i:s");
		...
		$mail->Body = "<h1>SQL error in AP Validation portal has occured consult error logs</h1><p>Location: {$location}</p><p>{$query}</p>";
		$mail->send();
	}

	function errorNotification2($email, $location, $query) {
		# send email notification whenever a sql error has occured
		require_once  'MailPack/PHPMailer/PHPMailerAutoload.php';
		require 'MailPack/phpCredential.php';
		$datein = date("Y-m-d"); 
		...
		$mail->setFrom("notification@sbe-ltd.ca", "AP Validation");
		$mail->Subject = "Error Notification: {$datein} {$timein}";
		$mail->Body = "<h1>SQL error in AP Validation portal has occured consult error logs</h1><p>Location: {$location}</p><p>".printArr($query)."</p>";
		$mail->send();
	}

	function saveInvoiceItems_Samsung($items, $id) {
		...
		foreach ($items as $key => $value) {
			...
			if(!$conn->query($query)) {
				...
				$errorData = [];
				$errorData['portal_name'] = "ap_validation - saveInvoiceItems";
				...
				errorNotification("cnayve@sbe.ltd.ca", "saveInvoiceItems_Samsung function", $query);
			}
			// $conn->query($query);
		} 
	...
	function saveInvoiceItems($supplier, $items, $id) {
		...
		switch (strtolower($supplier)) {
			case 'samsung':
				saveInvoiceItems_Samsung($items, $id);
				return true;
				break;

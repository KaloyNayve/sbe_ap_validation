	...
	*/		
	...
	if (isset($_POST["get_notes"])) {
		...
		$qry = "SELECT
					...
					to_char(added_date, 'HH:ii:ss') as added_time,
					ap_validation_id
	...
	if (isset($_POST['download_in_flow_report'])) {
	  ...
	  if (isset($_POST['editDocumentInfo'])) {
	  	...
	  	// generate data array for saving data
		$data = array_diff_key($input, array_flip(['index','document_file', 'editDocumentInfo', 'original_type', 'page']));
		$data['updated_by'] = $uname;
	  ...
	  if (isset($_POST['search_backups'])) {
	  	...
	  	$qry = "SELECT * FROM AP_VALIDATION 
	  			WHERE status not in ('to be received') "; // initial qry
	  	$prefix = "AND";

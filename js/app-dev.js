...
const reassignDocumentTypeForm = document.querySelector("#reassign_document_type-form");
const validateBtn_bottom = document.querySelector("#validate_btn-bottom");
const onholdDeleteForm = document.querySelector("#delete_on-hold_documents-form");
...
function loading() {
  ...
  function download(download_url, msg = "Document downloaded") {
    ...
    // Get excel files contents and output in the page
            ...
            case 0:            
              ...
              if (row.classList.contains("selected-row")) {
                ...
                if (ext === ".pdf") {                
                  ...
                  if (validateBtn_bottom) {
                    validateBtn_bottom.disabled = false;
                  }
          ...
          if (result.value) {
            $.ajax({
              ...
              data: {
                "delete_document" : "1",
                "document_id" : selected.id,
                "invoice_number" : selected.dataset.invoicenumber
              },
              success: function(data){
                if (data === 'success') {
                  Swal.fire(
        ...
        success: function(data){
          if (data === 'none found') {
            ...
            const html = res.map(row => {
              ...
              let delete_btn = '';
              // if added by user logged in or access is admin show delete admin
              const created_by = row.ADDED_BY.toLowerCase();            
              if (created_by === uname || access === 'admin') {
                delete_btn = `<div class="timeline-footer">
  ...
  function documentInformation() {
    //remove added elements if present
    const original_type = document.querySelector('#original_type');
    const page = document.querySelector('#page');  
    if (original_type) {
      page.remove();
      original_type.remove();
    }
    const selected = document.querySelector('#documentTable .selected-row');
    // get page name
    ...
    if (selected === null) {
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          document.querySelector("#sender").value = (res['SENDER_NAME'] == null) ? "Invoice Uploaded Manually" : res['SENDER_NAME'];
          document.querySelector("#subject").value = (res['EMAIL_SUBJECT'] == null) ? "Invoice Uploaded Manually" : res['EMAIL_SUBJECT'];
          document.querySelector("#attached_file").value = res['ATTACHED_DOCUMENT'];
          document.querySelector("#date_and_time").value = (res['SENT_DATE'] === null) ? res['DATE_CREATION'] : `${res['SENT_DATE']} ${res['SENT_TIME']}`;
          document.querySelector("#index").value = res['ID'];
  ...
  // Open change invoice number modal and populate information for that invoice
  function changeInvoiceNumber() {
    ...
    if (selected === null) {
        ...
        Swal.fire({
          ...
          // populate invoice number and id
        document.querySelector("#change_invoice_number").value = selected.dataset.invoicenumber;
        document.querySelector("#old_invoice_number").value = selected.dataset.invoicenumber;
        document.querySelector("#change_id").value = selected.id;
        $("#change_invoice_number_modal").modal();        
        
      }    
  }

  // open reassign document type modal and populate information for that invoice
  function reassignDocumentType() {
    const selected = document.querySelector('#documentTable .selected-row');
    if (selected === null) {
        // Sends error if nothing is selected
        Swal.fire({
        ...
        // populate invoice number and id
        document.querySelector("#reassign_document_type-invoice_number").value = selected.dataset.invoicenumber;
        document.querySelector("#reassign_document_type-document_type").value = selected.dataset.documenttype;
        document.querySelector("#reassign_document_type-id").value = selected.id;
        $("#reassign_document_type-modal").modal();
      }
  }

  // adding loading animation on submit 
  reassignDocumentTypeForm.addEventListener('submit', function(event) {
    loading();
  })

  // on hold delete load deletion reason modal
  function onhold_delete(){
    const selected = document.querySelector('#documentTable .selected-row');    
    if (selected === null) {
      // Sends error if nothing is selected
      Swal.fire({
      ...
      document.querySelector("#delete_on-hold_documents-comment").value = ""; //clear comments
      document.querySelector("#delete_on-hold_documents-id").value = selected.id;
      document.querySelector("#delete_on-hold_documents-invoice_number").value = selected.dataset.invoicenumber;
      $("#delete_on-hold_documents-modal").modal();   
    }
  }

  // on hold delete submit handler
  function onHoldDeleteSubmitHandler(event) {
    const selected = document.querySelector('#documentTable .selected-row');
    event.preventDefault();
    const form_data = new FormData(this);
    form_data.append("delete_on-hold_documents-submit", "1"); 
     // ajax call
     $.ajax({
      type: 'post',
        url: 'actions-dev.php',
        ...
        success: function(data){
          if (data === 'success') {
            ...
            showPdf("","viewerContainer");
            // hides modal
            $("#delete_on-hold_documents-modal").modal('hide');   
          } else {
            // Sends error if response is fail
            Swal.fire({
                icon: 'error',
  ...
  onholdDeleteForm.addEventListener('submit', onHoldDeleteSubmitHandler);

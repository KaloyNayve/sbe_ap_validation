<?php 
    ...
    $sel = "SELECT count(*) as allcount FROM AP_VALIDATION WHERE status = 'to be received' and deleted is null";
    $records = $conn->query($sel)->fetch(PDO::FETCH_ASSOC);
    $totalRecords = $records['ALLCOUNT'];

    ## Total number of record with filtering
    $sel = "SELECT count(*) as allcount FROM AP_VALIDATION WHERE status = 'to be received' and deleted is null".$searchQuery;
    $records = $conn->query($sel)->fetch(PDO::FETCH_ASSOC);
    ...
    on a.id = c.ap_validation_id WHERE a.status = 'to be received' and a.deleted is null {$searchQuery}  )T)T WHERE rowIndex > ".$row." AND rowIndex <= " .$last . ") order by ".$columnName." ".$columnSortOrder;    
    
    $empRecords = $conn->query($empQuery)->fetchAll(PDO::FETCH_ASSOC);
    $data = array();

    foreach ($empRecords as $key => $row) {
        $invoice_number = $row['INVOICE_NUMBER'] === null ? $row['ID'] : $row['INVOICE_NUMBER'];
        ...
        $data[] = array( 
            ...
            "renamed_attached_file" => $row['RENAMED_ATTACHED_FILE'],

<?php 
    ...
    ## If access is in Ap, Admin then get all on hold invoices, if not get corresponding document type
    if (in_array($access, array('admin', 'ap'))) {
       $document_type_qry = "";
    } else if ($access == "approver") {
         // get your documents on hold
         $document_type = getDocumentType($uname);
         $document_type_qry = " AND DOCUMENT_TYPE in ({$document_type}) ";
    }
    ...
    if($searchValue != ''){
        ...
        $searchQuery = " and (invoice_number like '%".$searchValue."%' or 
                LOWER(document_type) like '%".$searchValue."%' or 
                lower(supplier) like'%".$searchValue."%' or 
                invoice_date like'%".$searchValue."%') ";
    }    

    ## Total number of records without filtering
    $sel = "SELECT count(*) as allcount FROM AP_VALIDATION WHERE status = 'on hold' and deleted is null {$document_type_qry}";
    $records = $conn->query($sel)->fetch(PDO::FETCH_ASSOC);
    $totalRecords = $records['ALLCOUNT'];

    ## Total number of record with filtering
    $sel = "SELECT count(*) as allcount FROM AP_VALIDATION WHERE status = 'on hold' and deleted is null {$document_type_qry} ".$searchQuery;
    $records = $conn->query($sel)->fetch(PDO::FETCH_ASSOC);
    ...
    on a.id = c.ap_validation_id WHERE a.status = 'on hold' and a.deleted is null  {$document_type_qry}  {$searchQuery}  )T)T WHERE rowIndex > ".$row." AND rowIndex <= " .$last . ") order by ".$columnName." ".$columnSortOrder;    

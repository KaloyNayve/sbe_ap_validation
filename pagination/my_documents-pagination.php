<?php 
    ...
    $last = intval($row) + intval($rowperpage); //get the highest row index each page
    $uname = $_POST['uname']; // to get document type of user
...
//     ## Total number of records without filtering
    $sel = "SELECT count(*) as allcount FROM AP_VALIDATION WHERE status = 'in flow' and document_type in (" .$document_type. ") and deleted is null";
    $records = $conn->query($sel)->fetch(PDO::FETCH_ASSOC);
    $totalRecords = $records['ALLCOUNT'];

//     ## Total number of record with filtering
    $sel = "SELECT count(*) as allcount FROM AP_VALIDATION WHERE status = 'in flow' and deleted is null and document_type in (" .$document_type. ") ".$searchQuery;
    $records = $conn->query($sel)->fetch(PDO::FETCH_ASSOC);
...
//     //     on a.id = c.ap_validation_id WHERE a.status = 'in flow' and a.deleted is null {$searchQuery} order by ".$columnName." ".$columnSortOrder." )T)T WHERE rowIndex > ".$row." AND rowIndex <= " .$last;
    ...
    on a.id = c.ap_validation_id WHERE a.status = 'in flow' and a.deleted is null and document_type in (" .$document_type. ") {$searchQuery}  )T)T WHERE rowIndex > ".$row." AND rowIndex <= " .$last . ") order by ".$columnName." ".$columnSortOrder;    

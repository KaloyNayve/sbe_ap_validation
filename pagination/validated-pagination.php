<?php 
    ...
    $sel = "SELECT count(*) as allcount FROM AP_VALIDATION WHERE status = 'validated' and deleted is null";
    $records = $conn->query($sel)->fetch(PDO::FETCH_ASSOC);
    $totalRecords = $records['ALLCOUNT'];

    ## Total number of record with filtering
    $sel = "SELECT count(*) as allcount FROM AP_VALIDATION WHERE status = 'validated' and deleted is null".$searchQuery;
    $records = $conn->query($sel)->fetch(PDO::FETCH_ASSOC);
    ...
    on a.id = c.ap_validation_id WHERE a.status = 'validated' and a.deleted is null {$searchQuery}  )T)T WHERE rowIndex > ".$row." AND rowIndex <= " .$last . ") order by ".$columnName." ".$columnSortOrder;    

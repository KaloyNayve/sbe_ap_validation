<?php 
	...
	if (isset($_POST['download_in_flow_report'])) {
	  ...
	  if (isset($_POST['search_backups'])) {
		...
		$qry = "SELECT 
					A.*, 
					case when notes.notes is null then 0 else notes end as notes,
					case when attached_documents is null then 0 else attached_documents end as attached_documents
				FROM AP_VALIDATION A
				LEFT OUTER JOIN (select ap_validation_id, count(*) as notes from ap_validation_notes where deleted is null group by ap_validation_id) notes
				ON a.id = notes.ap_validation_id
				LEFT OUTER JOIN (select ap_validation_id, count(*) as attached_documents from ap_validation_attachments where deleted is null group by ap_validation_id) attachments
				on a.id = attachments.ap_validation_id
				WHERE A.status not in ('to be received') 
				and A.deleted is null ";
	  	$prefix = "AND";
	  	//generate search conditions in query
	  	if (isset($input['invoice_number']) && !empty($input['invoice_number'])) {
	  		$qry .= $prefix . " a.invoice_number = '{$input['invoice_number']}' ";
	  		$prefix = "AND";
	  	}

	  	if (isset($input['supplier_search']) && !empty($input['supplier_search'])) {
	  		$qry .= $prefix . " a.supplier = '{$input['supplier_search']}' ";
	  		$prefix = "AND";
	  	}
	  	...
	  	if (isset($input['invoice_date']) && !empty($input['invoice_date'])) {
			...
			$date_to = date("Y/m/d", strtotime($invoiceDate[1]) );  
	  		$qry .= $prefix . " a.invoice_date between '{$date_from}' and '{$date_to}' ";	  		
	  		$prefix = "AND";
		  }		  
